...
#
# Description:
# This script performed Gene Ontology analysis in neutrophils. 
...
red_quad_bp <- read.csv("input_data/enrichr_redQuad_top10_bp_v3.csv")
blue_quad_bp <- read.csv("input_data/enrichr_blueQuad_top10_bp_v3.csv")
...
lollipop_plot <- function(dat, gene_set, title, font_size, dot_size, positive) {
  ...
  # Save plot
  tiff_name <- paste("plots/24c5/mye/enrichr/", gene_set, "_", direction, "_lollipop_v3.tiff", sep = "")
...
# Generate plots
gobp_lollipop_up <- lollipop_plot(dat = red_quad_bp, gene_set = "gobp", title = "GO | Biological Processes", font_size = 14, dot_size = 12, positive = TRUE)
gobp_lollipop_down <- lollipop_plot(dat = blue_quad_bp, gene_set = "gobp", title = "GO | Biological Processes", font_size = 14, dot_size = 12, positive = FALSE)
